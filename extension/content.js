(() => {
  ...
  async function downloadLanguage(info, language) {
    ...
    downloadText(`${safeName(info.title)}.${language}.srt`, toSrt(sorted));
    return sorted.length;
  }

  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    if (message?.type === 'MNET_SUB_GET_INFO') {
      getInfo().then((info) => sendResponse({ ok: true, info })).catch((error) => sendResponse({ ok: false, error: error.message }));
      return true;
    }

    if (message?.type === 'MNET_SUB_DOWNLOAD') {
      (async () => {
        try {
          ...
          for (const language of selected) {
            const count = await downloadLanguage(info, language);
